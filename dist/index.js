...
var db_exports = {};
__export(db_exports, {
  addVsSignalStat: () => addVsSignalStat,
  ...
  getRecentConfluenceSignals: () => getRecentConfluenceSignals,
  getRecentSignals: () => getRecentSignals,
  getSnapshotHistory: () => getSnapshotHistory,
  getStrategyWinRateStats: () => getStrategyWinRateStats,
  getTelegramConfig: () => getTelegramConfig,
  getTodayStats: () => getTodayStats,
  getTradeCases: () => getTradeCases,
  getTrades: () => getTrades,
  getUserByOpenId: () => getUserByOpenId,
  getVsSignalStats: () => getVsSignalStats,
  getVsSignalWinRate: () => getVsSignalWinRate,
  insertAccountSnapshot: () => insertAccountSnapshot,
  insertBacktestResult: () => insertBacktestResult,
  insertTrade: () => insertTrade,
  loadVSLoginCredentials: () => loadVSLoginCredentials,
  recordConfluenceSignal: () => recordConfluenceSignal,
  recordSignal: () => recordSignal,
  saveVSLoginCredentials: () => saveVSLoginCredentials,
  updateStrategyConfig: () => updateStrategyConfig,
  updateVsSignalStat: () => updateVsSignalStat,
  upsertPosition: () => upsertPosition,
  upsertStrategyConfig: () => upsertStrategyConfig,
  upsertTelegramConfig: () => upsertTelegramConfig,
...
var DATABASE_URL, pool, dbInstance, memory, runtimeSecrets;
var init_db = __esm({
  "server/db.ts"() {
    "use strict";
    init_schema();
...
function connectSSE(path3, queryString, onEvent, onError) {
  let destroyed = false;
  let retryCount = 0;
  function connect() {
    if (destroyed) return;
    const url = `${STREAM_BASE}${path3}?${queryString}`;
    ...
    const req = https.request({
      ...
      if (res.statusCode !== 200) {
        console.error(`[ValueScan SSE] ${path3} HTTP ${res.statusCode}`);
        res.resume();
        scheduleRetry();
        return;
      }
...
var STREAM_BASE, marketSSEStop, tokenSSEStop;
var init_valueScanSSEService = __esm({
  "server/valueScanSSEService.ts"() {
    "use strict";
    init_env();
    init_db();
    init_schema();
    STREAM_BASE = "https://stream.valuescan.ai";
    marketSSEStop = null;
    tokenSSEStop = null;
  }
});
...
var valueScanService_exports = {};
__export(valueScanService_exports, {
  FUNDS_MOVEMENT_TYPE_MAP: () => FUNDS_MOVEMENT_TYPE_MAP,
  bootstrapValueScanService: () => bootstrapValueScanService,
  ensureVSToken: () => ensureVSToken,
  getAIMessages: () => getAIMessages,
  getChanceCoinList: () => getChanceCoinList,
  ...
  getVSTokenStatus: () => getVSTokenStatus,
  getWarnMessageWithToken: () => getWarnMessageWithToken,
  getWarnMessages: () => getWarnMessages,
  initVSTokenFromDB: () => initVSTokenFromDB,
  loginValueScan: () => loginValueScan,
  parseSignalContent: () => parseSignalContent,
  setVSToken: () => setVSToken,
...
async function requestValueScan(path3, init = {}, requireAuth = false) {
  const { apiKey, secretKey } = await getVsCredentials();
  const url = path3.startsWith("http") ? path3 : `https://api.valuescan.io${path3}`;
...
function parseSignalContent(content) {
...
async function getCoinSocialSentiment(symbol) {
  const normalized = normalizeSymbol3(symbol).replace(/USDT$/, "");
  try {
    const json2 = await requestValueScan(`/api/v1/market/social-sentiment?symbol=${normalized}`);
    return {
      code: 200,
      msg: "ok",
      data: {
        symbol: normalized,
        sentiment: Number(json2?.data?.sentiment ?? json2?.data?.score ?? 60),
        socialScore: Number(json2?.data?.socialScore ?? json2?.data?.score ?? 60),
        mentionCount: Number(json2?.data?.mentionCount ?? 120),
        source: "ValueScan"
      }
    };
  } catch {
...
var init_valueScanService = __esm({
  "server/valueScanService.ts"() {
    ...
    init_env();
    init_db();
    init_valueScanSSEService();
    TOKEN_REFRESH_INTERVAL_MS = 30 * 60 * 1e3;
    TOKEN_STALE_AFTER_MS = 28 * 60 * 1e3;
    FUNDS_MOVEMENT_TYPE_MAP = {
...
var paperTradingEngine_exports = {};
__export(paperTradingEngine_exports, {
  getEngineStatus: () => getEngineStatus,
  isEngineRunning: () => isEngineRunning,
  runPaperTradingCycle: () => runPaperTradingCycle,
  startPaperTradingEngine: () => startPaperTradingEngine,
  stopPaperTradingEngine: () => stopPaperTradingEngine
});
...
var BINANCE_BASE, engineRunning, engineTimer, lastCycleTime, lastSignalCount;
var init_paperTradingEngine = __esm({
  "server/paperTradingEngine.ts"() {
    "use strict";
    init_db();
    init_schema();
    init_valueScanService();
...
var dailyReportService_exports = {};
__export(dailyReportService_exports, {
  generateAndSendDailyReport: () => generateAndSendDailyReport,
  startDailyReportScheduler: () => startDailyReportScheduler,
  stopDailyReportScheduler: () => stopDailyReportScheduler
});
import { gte as gte2, and as and2, eq as eq5 } from "drizzle-orm";
async function sendTg3(message) {
  try {
    const cfg = await getTelegramConfig();
...
async function generateAndSendDailyReport() {
  ...
  const todayStart = new Date(utc8Today.getTime() - utc8Offset);
  let liveTotalPnl = 0;
  ...
  try {
    const liveTodayTrades = await db2.select().from(trades).where(and2(gte2(trades.openedAt, todayStart)));
    liveTotalTrades = liveTodayTrades.length;
    for (const t2 of liveTodayTrades) {
      const pnl = t2.pnl ?? 0;
      liveTotalPnl += pnl;
      if (pnl > 0) liveWinTrades++;
    }
    const cfgRows = await db2.select().from(strategyConfig).where(eq5(strategyConfig.isActive, true)).limit(1);
    const cfg = cfgRows[0];
  ...
  let paperBalance = 1e4;
  try {
    const paperTodayTrades = await db2.select().from(paperTrades).where(and2(gte2(paperTrades.closedAt, todayStart)));
    paperTotalTrades = paperTodayTrades.length;
    for (const t2 of paperTodayTrades) {
      const pnl = t2.pnl ?? 0;
      paperTotalPnl += pnl;
    ...
    const acctRows = await db2.select().from(paperAccount).where(eq5(paperAccount.id, 1)).limit(1);
    paperBalance = acctRows[0]?.balance ?? 1e4;
  } catch (e) {
  ...
  const msg = `\u{1F4CA} <b>\u6BCF\u65E5\u4EA4\u6613\u62A5\u544A - ${dateStr}</b>

\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501
\u{1F534} <b>\u5B9E\u76D8\uFF08${liveExchange}\uFF09</b>
\u{1F4B0} \u4ECA\u65E5\u76C8\u4E8F: <b>${liveTotalPnl >= 0 ? "+" : ""}$${liveTotalPnl.toFixed(2)}</b>
\u{1F4C8} \u4EA4\u6613\u6B21\u6570: ${liveTotalTrades} \u7B14
\u{1F3AF} \u80DC\u7387: ${liveWinRate}${liveWinRate !== "N/A" ? "%" : ""}
\u{1F4C9} \u6700\u5927\u56DE\u64A4: ${liveMaxDrawdown.toFixed(2)}%

\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501
\u{1F7E1} <b>\u6A21\u62DF\u76D8</b>
\u{1F4B0} \u4ECA\u65E5\u76C8\u4E8F: <b>${paperTotalPnl >= 0 ? "+" : ""}$${paperTotalPnl.toFixed(2)}</b>
\u{1F4C8} \u4EA4\u6613\u6B21\u6570: ${paperTotalTrades} \u7B14
\u{1F3AF} \u80DC\u7387: ${paperWinRate}${paperWinRate !== "N/A" ? "%" : ""}
\u{1F4C9} \u6700\u5927\u56DE\u64A4: ${paperMaxDrawdown.toFixed(2)}%
\u{1F4B5} \u6A21\u62DF\u4F59\u989D: $${paperBalance.toFixed(2)} USDT

\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501
\u23F0 \u62A5\u544A\u65F6\u95F4: ${now.toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" })}`;
  await sendTg3(msg);
}
function getMillisUntilNext2000CST() {
  const now = /* @__PURE__ */ new Date();
  const utc8Offset = 8 * 60 * 60 * 1e3;
  const utc8Now = new Date(now.getTime() + utc8Offset);
  ...
  if (next2000UTC <= now.getTime()) {
    return next2000UTC + 24 * 60 * 60 * 1e3 - now.getTime();
  }
...
function startDailyReportScheduler() {
  const scheduleNext = () => {
    ...
    console.log(`[DailyReport] \u4E0B\u6B21\u62A5\u544A\u65F6\u95F4: ${nextTime.toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" })} (${Math.round(msUntilNext / 6e4)} \u5206\u949F\u540E)`);
...
var dailyReportTimer;
var init_dailyReportService = __esm({
  "server/dailyReportService.ts"() {
    "use strict";
    init_db();
    init_schema();
...
async function fetchRSSFeed(source) {
  try {
    const res = await axios2.get(source.url, {
...
async function cgFetchV4(path3, params = {}) {
  const url = new URL(`${COINGLASS_BASE_V4}${path3}`);
...
var BinanceService = class {
  ...
  async request(path3, method = "GET", params = {}, signed = false) {
    ...
    const url = `${this.baseUrl}${path3}${signedQuery ? `?${signedQuery}` : ""}`;
    const headers = {};
...
var OKXService = class {
  ...
  /** 构建请求头 */
  buildHeaders(method, path3, body = "") {
    const timestamp2 = this.getTimestamp();
    const sign = this.sign(timestamp2, method, path3, body);
    const headers = {
  ...
  async get(path3, params = {}) {
    ...
    const fullPath = queryString ? `${path3}?${queryString}` : path3;
  ...
  async post(path3, body) {
    ...
    const headers = this.buildHeaders("POST", path3, bodyStr);
    const res = await this.client.post(path3, body, { headers });
    if (res.data.code !== "0") {
  ...
  async ping() {
    try {
      await axios3.get(`${OKX_BASE}/api/v5/public/time`, { timeout: 5e3 });
      return true;
    } catch {
      return false;
    }
...
var BybitService = class {
  ...
  constructor(config) {
    ...
    this.client = axios4.create({ baseURL, timeout: 1e4 });
  }
  ...
  async request(method, path3, params = {}) {
    ...
    const url = method === "GET" ? `${path3}?${queryString}` : path3;
...
var GateService = class {
  ...
  sign(method, path3, queryString, body = "") {
    const ts = String(Math.floor(Date.now() / 1e3));
    const bodyHash = crypto4.createHash("sha512").update(body).digest("hex");
    const signString = [method, path3, queryString, bodyHash, ts].join("\n");
    const sign = crypto4.createHmac("sha512", this.secretKey).update(signString).digest("hex");
    return { ts, sign };
  }
  async request(method, path3, query = {}, body) {
    const queryString = buildQuery(query);
    const payload = body ? JSON.stringify(body) : "";
    const { ts, sign } = this.sign(method, path3, queryString, payload);
    const url = `${this.baseUrl}${path3}${queryString ? `?${queryString}` : ""}`;
    const res = await fetch(url, {
      method,
      headers: {
        Accept: "application/json",
...
var BitgetService = class {
  ...
  constructor(config) {
    ...
    this.secretKey = config.secretKey;
    this.passphrase = config.passphrase;
    this.client = axios5.create({ baseURL: BITGET_BASE, timeout: 1e4 });
  }
  sign(timestamp2, method, path3, body) {
    const message = `${timestamp2}${method.toUpperCase()}${path3}${body}`;
...
setTimeout(async () => {
  try {
    ...
    const { strategyConfig: strategyConfig2 } = await Promise.resolve().then(() => (init_schema(), schema_exports));
    const { eq: eq6 } = await import("drizzle-orm");
    const { startPaperTradingEngine: startPaperTradingEngine2, isEngineRunning: isEngineRunning2 } = await Promise.resolve().then(() => (init_paperTradingEngine(), paperTradingEngine_exports));
    const db2 = await getDb2();
    if (!db2) return;
    const rows = await db2.select().from(strategyConfig2).where(eq6(strategyConfig2.isActive, true)).limit(1);
    const cfg = rows[0];
    if (cfg?.autoTradingEnabled && !isEngineRunning2()) {
      startPaperTradingEngine2();
...
setTimeout(async () => {
  try {
    const { bootstrapValueScanService: bootstrapValueScanService2 } = await Promise.resolve().then(() => (init_valueScanService(), valueScanService_exports));
    const status = await bootstrapValueScanService2();
...
setTimeout(async () => {
  try {
    const { startDailyReportScheduler: startDailyReportScheduler2 } = await Promise.resolve().then(() => (init_dailyReportService(), dailyReportService_exports));
