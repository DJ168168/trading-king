...
var OPEN_API_BASE = "https://api.valuescan.io/api";
async function requestOpenApi(path2, body = {}) {
  const { apiKey, secretKey } = await getVsCredentials();
...
\u{1F550} ${(/* @__PURE__ */ new Date()).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" })}`;
  ...
  quant: router({
    ...
    signal: publicProcedure.query(async () => {
      try {
        const res = await fetch("http://47.239.72.211:3888/api/best", { signal: AbortSignal.timeout(8e3) });
    ...
    status: publicProcedure.query(async () => {
      try {
        const res = await fetch("http://47.239.72.211:3888/api/status", { signal: AbortSignal.timeout(8e3) });
        return { success: true, data: await res.json() };
      } catch (e) {
        return { success: false, data: null, error: e.message };
      }
    ...
    history: publicProcedure.query(async () => {
      try {
        const res = await fetch("http://47.239.72.211:3888/api/history", { signal: AbortSignal.timeout(8e3) });
        return { success: true, data: await res.json() };
      } catch (e) {
        return { success: false, data: [], error: e.message };
      }
    ...
    toggle: publicProcedure.mutation(async () => {
      try {
        const res = await fetch("http://47.239.72.211:3888/api/toggle", { signal: AbortSignal.timeout(8e3) });
        return { success: true, data: await res.json() };
      } catch (e) {
        return { success: false, data: null, error: e.message };
      }
